...
var WebServer = _interopRequire(require("./WebServer"));

var Model = _interopRequire(require("./Model"));

class TeamServer extends WebServer {
	init() {
		this.addStaticHandler(`${ __dirname }/../../client`);
		this.addInterceptor(this.modelHandlerInterceptor);
		this.routes().get("/api/teams", this.getTeams).post("/api/teams", this.selectTeam);
	}
